...
/* [Min]
SHA1 的绝大多数代码与 MD5 相同，填充规则也一样，只是摘要长度不同，最终字节序为 bigEndian
*/

// [Min] SHA-1 摘要信息长度为固定的20个字节
...
// [Min] 与 MD5 结构类似，只是摘要长度不同
type digest struct {
	h   [5]uint32 // [Min] 20个字节160位长
	x   [chunk]byte
	nx  int
	len uint64
}
...
func (d *digest) constSum() [Size]byte {
	...
	// [Min] 填充分块中待填充的数据长度
	nx := byte(d.nx)
	// [Min] 如果小于56，说明填充一个分块即可，
	// [Min] t<0，转为 int8，最高位为1，向右移7位，变为11111111，再转为 uint8
	// [Min] 用于标记一个填充分块即可
	...
	for i := byte(0); i < chunk; i++ {
		// [Min] 如果是当前字节属于等待被填充的那一部分，mask 为0xFF，过了这一部分就是0x00
		mask := byte(int8(i-nx) >> 7) // 0x00 after the end of data

		// if we reached the end of the data, replace with 0x80 or 0x00
		// [Min] 还没到填充位时，0x00 | d.x[i]，还是 d.x[i]，separator 保持不变
		// [Min] 到第一个填充位时，0xFF & 0x80 | 0x00 , d.x[i] 变为 0x80
		// [Min] 到后续填充位时，0xFF & 0x00 | 0x00 , d.x[i] 变为 0x00
		...
		if i >= 56 {
			...
			// [Min] 如果只需1个填充块，说明 nx < 56，需要在此块末尾填入长度
