...
// [Min] 将字符串表示的数值解析为对应的无符号的整数，base是进制，bitSize为字宽
...
// [Min] 将字符串表示的数值解析为对应的有符号的整数，base是进制，bitSize为字宽
...
// [Min] Atoi等效于ParseInt(s,10,0),即将字符串表示的数值转化为有符号的10进制int整数
func Atoi(s string) (int, error) {
	...
	if intSize == 32 && (0 < sLen && sLen < 10) ||
		...
		// [Min] pow(2,31) 的位数是10，pow(2,63)的位数是19,对于没有溢出可能的字符串，直接快速转化
		s0 := s
		if s[0] == '-' || s[0] == '+' {
			s = s[1:]
			if len(s) < 1 {
