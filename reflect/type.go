...
// [Min] Type是一个接口，实现该接口的是后续定义好的各种类型对象xxxType
type Type interface {
	...
	// It panics if the type's Kind is not Struct.
	// [Min] 根据索引树返回StructField，索引树一层为一个嵌套
	...
	// [Min] 根据给定的匹配函数，以字段名来匹配，返回StructField
	...
	// [Min] 注意这里Type的 Len() 方法和Value的Len()不同，只能用于数组，不能用于切片
	Len() int

	// NumField returns a struct type's field count.
	// It panics if the type's Kind is not Struct.
	// [Min] 返回结构体字段的数量，包含不可导出的字段
	NumField() int

	// NumIn returns a function type's input parameter count.
	// It panics if the type's Kind is not Func.
	// [Min] 返回Func输入字段的个数，可变参数算一个同类型的切片字段
	NumIn() int
