...
// pointer receiver.
// [Min] 如果CanAddr()为真，返回v的地址，否则panic
...
// [Min] 如果v是bool类型的值，则返回对应的bool值，否则panic
...
// [Min] 如果v是Func类型的值，以in为参数调用该函数，返回对应的输出Value
...
// As in Go, each input argument must be assignable to the
// type of the function's corresponding input parameter.
// [Min] 如果v是Func类型的值，且参数包含可变参数，以in为参数调用该函数，返回对应的输出Value
...
// [Min] 如果v的类型接口或指针，返回接口内包含的值，或指针指向的值
// [Min] 这里和Type的Elem不同，不能对数组，切片，通道，map类型的值使用Elem方法
...
// [Min] 如果v是float32或float64类型的值，返回对应的float64值
...
// [Min] 如果v是数组，切片，返回第i个元素的值
// [Min] 如果v是字符串，返回第i个字节的值
...
// [Min] 如果v是数组，切片，通道，map，字符串，返回对应的长度
...
// [Min] v为map类型的值，根据key的值返回对应的map值
...
// [Min] 如果v是Func类型的值，返回第i个可导出方法对应的值
...
// [Min] 如果v是Func类型的值，根据方法名返回对应的方法值
...
// It panics if v's Kind is not Struct.
// [Min] 如果v是结构体类型的值，返回字段的数量，包含不可导出的字段
...
// It panics if v's Kind is not Complex64 or Complex128.
// [Min] 如果v是虚数类型的值，判断虚数x是否可以导致v溢出，主要是v为complex64的时候
...
// [Min] 如果v是通道，map，指针，函数，切片，返回其对应的指针值
...
// [Min] v为切片类型的值，n不能为负，且n不能超过切片的容量，将该切片的长度改为n
...
// greater than the capacity of the slice.
// [Min] v为切片类型的值，n不小于当前的长度，不大于当前的容量，按n设置容量
...
// [Min] v是数组，切片，字符串类型的值，且v可寻址，返回索引i,j对应部分的切片值
...
// or if the indexes are out of bounds.
// [Min] v是数组或切片类型的值，返回由i，j，k确定的新切片的值
// [Min] j-i为长度，k-i为容量
...
// [Min] v为非阻塞通道类型的值，从通道中获取一个值，ok为真表示获取成功
...
// [Min] dst，src均为数组，切片类型的值，将src拷贝至dst中
// [Min] 当dst中的元素为byte时，src可以为字符串类型
...
// [Min] 对给定的select case进行select 操作，返回执行的case的索引
// [Min] 如果是一个接受操作，则同时返回接受的值和ok
