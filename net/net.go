...
// [Min] 将 conn 的底层文件描述符拷贝并以 *os.File 返回，两者并不等价
...
// [Min] conn 实现了buffersWriter接口，将 Buffers 中的内容写入实现了该接口的对象
// [Min] writev_unix.go 具体实现
...
func (v *Buffers) WriteTo(w io.Writer) (n int64, err error) {
	// [Min] 先判断 w 是否实现了 buffersWriter 接口，如果实现了，直接调用 writeBuffer 方法写入 w 中
	// [Min] 如果 w 是一个 conn，则会调用此方法
